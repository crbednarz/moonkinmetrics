...
func TestGetSingeLoadout(t *testing.T) {
	...
	if len(responses[0].Loadout.ClassNodes) != 29 {
		t.Errorf("expected 29 class nodes, got %d", len(responses[0].Loadout.ClassNodes))
	}

	if len(responses[0].Loadout.SpecNodes) != 28 {
		t.Errorf("expected 28 spec nodes, got %d", len(responses[0].Loadout.SpecNodes))
	}
