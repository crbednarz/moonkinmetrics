...
@click.group()
@click.option(
    "--output", "-o", "output_path", type=click.Path(file_okay=False), default="ui/wow/"
)
@click.option(
    "--client-id", "client_id", default=lambda: os.environ.get("WOW_CLIENT_ID", "")
)
@click.option(
    "--client-secret",
    "client_secret",
...
@cli.command()
@click.argument(
    "bracket", type=click.Choice(["2v2", "3v3", "shuffle", "rbg"]), required=True
)
