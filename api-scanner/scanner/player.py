...
class PlayerLink:
    ...
    def profile_resource(self) -> str:
        return f"/profile/wow/character/" f"{self.realm.slug}/{self.name.lower()}"
...
async def get_player_loadouts(
    ...
    urls_with_context = [
        (player.specialization_resource, (player, context))
...
def _deserialize_json_loadout(
    json_loadout: dict, class_name: str, spec_name: str
) -> PlayerLoadout:
    class_nodes = []
    ...
    for raw_talent in json_loadout["pvp_talent_slots"]:
        pvp_talents.append(LoadoutPvpTalent(raw_talent["selected"]["talent"]["id"]))
...
def _get_active_loadout(
    player: PlayerLink, spec_name: str, specialization_resource_response: dict
) -> dict:
    ...
    if pvp_talents is None:
        raise RuntimeError(f"Unable to find pvp talents for {player.full_name}")
...
def get_realms(client: Client) -> list[Realm]:
    ...
    return [
        Realm(realm["id"], realm["name"], realm["slug"], realm["key"]["href"])
